...
const Navbar = () => {
  ...
  return (
    <nav className={`
      ${styles.paddingX} w-full items-center flex py-5 fixed top-0 z-20 bg-primary
    `}>
      <div className='w-full flex justify-between items-center max-w-7x1 mx-auto'>
        <Link to={'/'} className='flex items-center gap-2' onClick={()=>{
          ...
          window.scrollTo(0,0)
            ...
            <p className='text-white text-[18px] font-bold cursor-pointer flex'>John &nbsp;
            <span className='sm:block hidden '> | Web Developer</span></p>
          </Link>

          <ul className='list-none hidden sm:flex flex-row gap-10'>
            {navLinks.map(link=><li key={link.id} className={`
